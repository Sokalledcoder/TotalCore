...
class ObservationBuilder:
    def __init__(self, config: ObservationConfig, account_scale: float, max_position: float):
        self.config = config
        self.account_scale = account_scale
        self.max_position = max_position
        self.market_feature_count = 5 + (1 if config.include_returns else 0)
    ...
    def build(self, frame: pd.DataFrame, account: AccountState) -> dict[str, np.ndarray]:
        ...
        if self.config.include_returns:
            ...
            market = np.concatenate([market, returns], axis=1)
