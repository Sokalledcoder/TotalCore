---
source: https://tradingview.github.io/lightweight-charts/tutorials/customization/data-points
scraped_at: 2025-12-01T16:03:47.499771
---
...
If we take a look at the interface for the candlestick data here: CandlestickData, we can see a few properties related to color: `color`
...
We are going to change the color of all data points which have a `close`

value over `205`

to orange. We will use the map higher-order function to go through each data point and either leave it alone if it is below `205`

or otherwise add the `color`

and `wickColor`

properties (set to 'orange'). We don't need to set the `borderColor`

because we disabled that in an earlier step.
