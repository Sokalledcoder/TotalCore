---
source: https://tradingview.github.io/lightweight-charts/tutorials/customization/finishing-touches
scraped_at: 2025-12-01T16:03:48.399962
---
...
It is possible to specify a custom font to use within the Lightweight Charts™. As an example we are going to load the `Roboto`

font from Google Fonts by adding the following tags within the `<head>`

element on the page (for example, just below the `<title>`
...
We can then tell Lightweight Charts™ to use that specific font by setting the `fontFamily`

property within the `layout`

property of the chart options as follows:
