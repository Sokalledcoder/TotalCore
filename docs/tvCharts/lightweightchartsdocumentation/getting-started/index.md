...
scraped_at: 2025-12-01T14:31:37.318375
---
...
To support previous revisions, you can set up a transpilation process for the `lightweight-charts`

package in your build system using tools such as Babel.
If you encounter any issues, open a GitHub issue with detailed information, and we will investigate potential solutions.

## Installation

To set up the library, install the `lightweight-charts`
...
file - The https://www.tradingview.com link
...
To create a chart, use the `createChart`

function. You can call the function multiple times to create as many charts as needed:
...
As a result, `createChart`

returns an `IChartApi`

object that allows you to interact with the created chart.
...
To create a series, use the `addSeries`

method from `IChartApi`
...
To set the data to a series, you should call the `ISeriesApi.setData`
...
To do this, call the `ISeriesApi.update`

method that allows you to update the last data item or add a new one.
...
We do not recommend calling `ISeriesApi.setData`

to update the chart, as this method replaces all series data and can significantly affect the performance.